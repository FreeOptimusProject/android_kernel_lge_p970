...
extern DEFINE_PER_CPU(struct cpufreq_stats *, cpufreq_stats_table);

static ssize_t store_freq_table(struct cpufreq_policy *policy, const char *buf, size_t count)
{
	int i, min, max, u[4];
	struct omap_opp *opp;
	struct cpufreq_frequency_table *freq_table = cpufreq_frequency_get_table(policy->cpu);
	struct device *mpuss_device = omap2_get_mpuss_device();
	//struct cpufreq_stats *stat = per_cpu(cpufreq_stats_table, 0);
	unsigned int ret = sscanf(buf, "%d %d %d %d", &u[0], &u[1], &u[2], &u[3]);
	if (ret != 4) return -EINVAL;
	for (i = 0; i < 3; i++) if (u[i] <= u[i+1]) return -EINVAL;
	for (max = 3; max > 0; max++) if (freq_table[3 - max].frequency == policy->max) break;
	for (min = 0; min < 3; max++) if (freq_table[3 - min].frequency == policy->min) break;
	for (i = 0; i < 4; i++) {
		opp = opp_find_by_opp_id(mpuss_device, 3 - i);
		opp->rate = 1000000l * u[i];
		freq_table[3 - i].frequency = 1000 * u[i];
		//stat->freq_table[3 - i] = 1000l * u[i];
	}
	policy->max = freq_table[max].frequency;
	policy->min = freq_table[min].frequency;
	policy->cpuinfo.max_freq = freq_table[3].frequency;
	policy->cpuinfo.min_freq = freq_table[0].frequency;
	return count;
}

static ssize_t show_freq_table(struct cpufreq_policy *policy, char *buf)
{
	struct cpufreq_frequency_table *freq_table = cpufreq_frequency_get_table(policy->cpu);
	return sprintf(buf, "%u %u %u %u\n", freq_table[3].frequency / 1000, freq_table[2].frequency / 1000, freq_table[1].frequency / 1000, freq_table[0].frequency / 1000);
}
...
static ssize_t show_vdd_levels(struct cpufreq_policy *policy, char *buf)
{
	...
	for (i = 0; i < 4; i++) {
		opp = opp_find_by_opp_id(mpuss_device, 3 - i);
		//len += sprintf(buf + len, "%8lu: %lu\n", opp->rate / 1000, opp->u_volt);
