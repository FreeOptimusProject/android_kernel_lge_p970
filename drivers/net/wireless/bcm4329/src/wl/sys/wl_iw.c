...
static int wl_keep_alive_set(struct net_device *dev)
{
	const char              *str;
	...
	char buf[100]  = {0};
	uint8 * temp_mac;
	uint8 keep_alive_pkt_data[]={	0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x45, 0x00,
		0x00, 0x1e, 0x00, 0x00, 0x40, 0x00, 0x40, 0x11, 0xc5, 0x2a,
		0x0a, 0x88, 0x30, 0x70, 0x0a, 0x88, 0x30, 0x25, 0x13, 0xc4,
		0x13, 0xc4, 0x00, 0x0a, 0x00, 0x00, 0x0a, 0x0d};

	temp_mac = keep_alive_pkt_data + 6;
	memcpy(temp_mac , dev->dev_addr, 6);

	WL_TRACE(("dest mac [%02x][%02x][%02x][%02x][%02x][%02x]\n",keep_alive_pkt_data[0]
				,keep_alive_pkt_data[1],keep_alive_pkt_data[2],keep_alive_pkt_data[3]
				,keep_alive_pkt_data[4],keep_alive_pkt_data[5]));
	WL_TRACE(("source mac [%02x][%02x][%02x][%02x][%02x][%02x]\n",keep_alive_pkt_data[6]
				,keep_alive_pkt_data[7],keep_alive_pkt_data[8],keep_alive_pkt_data[9]
				,keep_alive_pkt_data[10],keep_alive_pkt_data[11]));
	...
	keep_alive_pkt.period_msec = htod32(290000);		//period : 290sec
	buf_len = str_len + 1;

	keep_alive_pkt.len_bytes =
		htod16(sizeof(keep_alive_pkt_data));

	memcpy(keep_alive_pktp->data, &keep_alive_pkt_data,sizeof(keep_alive_pkt_data));
	...
	if( (ret= dev_wlc_ioctl(dev, WLC_SET_VAR, buf, buf_len)) < 0)
		printk(KERN_ERR "%s:keep_alive set failed. ret[%d]\n",__FUNCTION__, ret);
...
static void wl_iw_send_scan_complete(iscan_info_t *iscan)
{
	...
	char extra[IW_CUSTOM_MAX + 1];
		...
		wireless_send_event(iscan->dev, SIOCGIWSCAN, &wrqu, NULL); // blocking Warning message : extra -> NULL
...
#endif	/* CONFIG_LGE_BCM432X_PATCH */
#ifndef SANDGATE2G
// blocking Warning message : extra -> NULL
	if (cmd)
	{
		if (cmd == SIOCGIWSCAN)
		{
